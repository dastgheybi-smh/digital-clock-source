...
from interface import Interface, upk, downk, esck, okk, menu_handler, menu_item_shorter, start_menu
import ujson
from lcd import *
...
def show_clock():
    ...
    if settings["Time"]["Watchface"][0] == "Character":
        if settings["Time"]["Show Temp & Hum"]:
            ...
            displayString(3, 0, f"              {temp}{d}{chr(0b11011111)}")
            displayString(4, 0, f"hum: {round(hum)}%")
            displayString(2, 0, "                    ")
    if settings["Time"]["Watchface"][0] == "7 Seg.":
        if settings["Time"]["Show Temp & Hum"]:
            ...
            if settings["Time"]["Farenhite Temp"]:
                temp = round((temp * 1.8 + 32), 1)
            displayString(4, 15, f"{temp}{d}{chr(0b11011111)}")
    ...
    try:
        while not okk.value():
            ...
            if mapping:
                ...
                break
            if settings["Time"]["Watchface"][0] == "Character":
                displayString(1, 0, f"            {clk[3]}:{clk[4]}:{clk[5]}")
                if settings["Time"]["Show Date"]:
                    if settings["Time"]["Persian Calender"]:
                        converted_date = convert_date((int(clk[0]), int(clk[1]), int(clk[2])))
                        displayString(2, 0, f"{converted_date[0]}/{converted_date[1]}/{converted_date[2]}")
                    else:
                        displayString(2, 0, f"{clk[0]}/{clk[1]}/{clk[2]}")
            ...
            if settings["Time"]["Watchface"][0] == "7 Seg.":
                ...
                if settings["Time"]["Show Date"]:
                    if settings["Time"]["Persian Calender"]:
                        converted_date = convert_date((int(clk[0]), int(clk[1]), int(clk[2])))
                        displayString(1, 15, f"{converted_date[0]}")
                        displayString(2, 15, f"{converted_date[1]}/{converted_date[2]}")
                    else:
                        displayString(1, 15, f"{clk[0]}")
                        displayString(2, 15, f"{clk[1]}/{clk[2]}")
                if settings["Time"]["Show Temp & Hum"]:
                    if i == 90:
                        i = 0
                        temp, hum = mes()
                        displayString(4, 15, f"{temp}{chr(0b11011111)}")
            i += 1
            sleep(0.15)
...
def OS():
    if update_setting_metadata or not exists("settings.json"):
        with open("settings.json", "w") as file:
            ujson.dump({"Time": {"Watchface": ["Character", "Character", "7 Seg.", "7 Seg.(with secs)"], "Farenhite Temp": False, "Show Date": True, "Persian Calender": True, "Show Temp & Hum": True}, "Battery Saving": {"B.S. Mode": ["Off", "Off", "Ultra B.S.", "Auto B.S."]}}, file)
