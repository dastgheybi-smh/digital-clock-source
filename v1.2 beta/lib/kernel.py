...
def _7s(number:int | str, x: int=0, codec: Codec7s=DefaultCodec7sv2):
    ...
    for i in range(len(ln)):
        ...
        if type(num) == int:
            ...
            charl = codec.items[n]
            got += 3
        lcd.lcd_show(1, prev_got+x, f"{charl[0]}", _7s=True)
        lcd.lcd_show(2, prev_got+x, f"{charl[1]}", _7s=True)
        lcd.lcd_show(3, prev_got+x, f"{charl[2]}", _7s=True)
        lcd.lcd_show(4, prev_got+x, f"{charl[3]}", _7s=True)
    if is_num:
        lcd.sevenSegmentRefresh(num)
